...
chicago2025 <- read_csv("data/source/recent/chicago2025.csv", 
                        col_types = cols(ID = col_character(), 
                                         Arrest = col_character(), Domestic = col_character(), 
                                         Ward = col_character(), `Community Area` = col_character(), 
                                         `X Coordinate` = col_skip(), `Y Coordinate` = col_skip()))
...
#add in ridership by year
download.file("https://data.cityofchicago.org/api/views/6iiy-9s97/rows.csv","ridership.csv")
ridership <- read_csv("ridership.csv")
ridership$service_date <- as.Date(ridership$service_date, format = "%m/%d/%Y")
ridership$year <- format(as.Date(ridership$service_date, format = "%Y-%m-%d"), "%Y")
ridership2019 <- ridership %>% filter(year == "2019")
ridership2019 <- sum(ridership2019$total_rides)
ridership2020 <- ridership %>% filter(year == "2020")
ridership2020 <- sum(ridership2020$total_rides)
ridership2021 <- ridership %>% filter(year == "2021")
ridership2021 <- sum(ridership2021$total_rides)
ridership2022 <- ridership %>% filter(year == "2022")
...
violent_transit <- violent_transit %>% mutate(rate19 = round(total19/ridership2019*1000000,1))
violent_transit <- violent_transit %>% mutate(rate20 = round(total20/ridership2020*1000000,1))
